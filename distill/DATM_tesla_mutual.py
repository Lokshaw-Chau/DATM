...
class DatasetDistiller():
    def __init__(self, args, ipc, channel, im_size, num_classes, tag=None):
        self.args = args
        ...
        self.optimizer_y = None
        self.model_eval_pool = get_eval_pool(args.eval_mode, args.model, args.model)
        self.best_acc = {m: 0 for m in self.model_eval_pool}
        self.best_std = {m: 0 for m in self.model_eval_pool}
        self.ipc = ipc
        self.tag = tag

    def get_images(self, c, n, indices_class, images_all):  # get random n images from class c
        idx_shuffle = np.random.permutation(indices_class[c])[:n]
        return images_all[idx_shuffle]

    def init_synthesis(self, img_all, indices_class, expert_files, buffer):
        args = self.args
        self.label_syn = torch.tensor([ [i] * self.ipc for i in range(self.num_classes)], dtype=torch.long, requires_grad=False, device=args.device).view(-1) # [0,0,0, 1,1,1, ..., 9,9,9]
        self.image_syn = torch.randn(size=(self.num_classes * self.ipc, self.channel, self.im_size[0], self.im_size[1]), dtype=torch.float)
        self.syn_lr = torch.tensor(args.lr_teacher).to(args.device)
        # init images
        if args.pix_init == 'real':
            print('initialize synthetic data from random real images')
            for c in range(self.num_classes):
                self.image_syn.data[c * self.ipc:(c + 1) * self.ipc] = self.get_images(c, self.ipc, indices_class, img_all).detach().data
            ...
            if cfg.Initialize_Label_With_Another_Model:
                Temp_net = get_network(args.Initialize_Label_Model, self.channel, self.num_classes, self.im_size, dist=False).to(device)  # get a random model
            else:
                Temp_net = get_network(args.model, self.channel, self.num_classes, self.im_size, dist=False).to(device)  # get a random model
            Temp_net.eval()
            ...
            for c in range(self.num_classes):
                data_for_class_c = self.get_images(c, len(indices_class[c]), indices_class, img_all).detach().data
                ...
                while len(index)<self.ipc:
                    ...
                    for i in range(len(prediction_class)):
                        if prediction_class[i]==c and len(index)<self.ipc:
                            index.append(batch_size*select_times+i)
                            index=list(set(index))
                    select_times+=1
                    if len(index) == self.ipc:
                        temp_img = torch.index_select(data_for_class_c, dim=0, index=torch.tensor(index))
                        break
                self.image_syn.data[c * self.ipc:(c + 1) * self.ipc] = temp_img.detach()
        else:
            print('initialize synthetic data from random noise')

        # init optimizers
        self.image_syn = self.image_syn.detach().to(args.device).requires_grad_(True)
        self.syn_lr = self.syn_lr.detach().to(args.device).requires_grad_(True)
        ...
        if args.pix_init == "samples_predicted_correctly":
            ...
            for i in range(len(label_expert_files)):
                ...
                for j in Temp_Buffer:
                    temp_logits = None
                    for select_times in range((len(self.image_syn)+batch_size-1)//batch_size):
                        current_data_batch = self.image_syn[batch_size*select_times : batch_size*(select_times+1)].detach().to(device)
            ...
            true_labels = self.label_syn.cpu()
            ...
            Temp_net = get_network(args.model, self.channel, self.num_classes, self.im_size, dist=False).to(args.device)  # get a random model
            Temp_net = ReparamModule(Temp_net)
            if args.distributed:
                Temp_net = torch.nn.DataParallel(Temp_net)
            Temp_net.eval()
            # FIXME buffer & expert files ? 
            Temp_params = buffer[0][-1]
            Initialize_Labels_params = torch.cat([p.data.to(args.device).reshape(-1) for p in Temp_params], 0)
            if args.distributed:
                Initialize_Labels_params = Initialize_Labels_params.unsqueeze(0).expand(torch.cuda.device_count(), -1)
            Initialized_Labels = Temp_net(self.image_syn, flat_param=Initialize_Labels_params)

        acc = np.sum(np.equal(np.argmax(Initialized_Labels.cpu().data.numpy(), axis=-1), self.label_syn.cpu().data.numpy()))
        print('InitialAcc:{}'.format(acc/len(self.label_syn)))

        self.label_syn = copy.deepcopy(Initialized_Labels.detach()).to(args.device).requires_grad_(True)
        self.label_syn.requires_grad=True
        self.label_syn = self.label_syn.to(args.device)
        

        self.optimizer_y = torch.optim.SGD([self.label_syn], lr=args.lr_y, momentum=args.Momentum_y)
        vs = torch.zeros_like(self.label_syn)
        accumulated_grad = torch.zeros_like(self.label_syn)
        last_random = 0
    ...
    def train_iter(self, it, expert_trajectory, criterion, images_partner, labels_partner, lr_partner):
        args = self.args
        wandb.log({"Synthetic_LR"+self.tag: self.syn_lr.detach().cpu()}, step=it)
        student_net = get_network(args.model, self.channel, self.num_classes, self.im_size, dist=False).to(args.device)  # get a random model
        ...
        syn_image_gradients = torch.zeros(self.image_syn.shape).to(args.device)
        syn_label_gradients = torch.zeros(self.label_syn.shape).to(args.device)
        x_list = []
        original_x_list = []
        y_list = []
        original_y_list = []
        ...
        for step in range(args.syn_steps):
            ...
            student_params.append(student_params[-1] - self.syn_lr.item() * detached_grad)
        ...
        for i in range(args.syn_steps):
            ...
            single_term = self.syn_lr.item() * (target_params - starting_params)
            square_term = (self.syn_lr.item() ** 2) * gradient_sum
        ...
        self.image_syn.grad = syn_image_gradients
        self.label_syn.grad = syn_label_gradients

        grand_loss = starting_params - self.syn_lr * gradient_sum - target_params
        grand_loss = grand_loss.dot(grand_loss) / param_dist

        lr_grad,  = torch.autograd.grad(grand_loss, self.syn_lr)
        self.syn_lr.grad = lr_grad
        ...
        # else:
        #     wandb.log({"falts"+self.tag: start_epoch}, step=it)

        wandb.log({"Grand_Loss"+self.tag: grand_loss.detach().cpu(),
                   "Start_Epoch"+self.tag: start_epoch}, step=it)

        for _ in student_params:
            del _
        
        # mutual loss
        # get a random model from trajectory file
        model_index = np.random.randint(0, len(expert_trajectory))
        model_params = expert_trajectory[model_index]
        model_params = torch.cat([p.data.to(args.device).reshape(-1) for p in model_params], 0)
        # FIXME large batch size
        output = student_net(syn_images, flat_param=model_params)
        output_partner = student_net(images_partner, flat_param=model_params)
        if args.mutual_dist == 'KL':
            # calculate KL divergence
            mutual_loss = F.kl_div(F.log_softmax(output, dim=1), F.softmax(output_partner, dim=1), reduction='batchmean')
        # TODO gradient dist
        else:
            print('Mutual loss not implemented!')
            mutual_loss = 0
        # backward to images(and labels)
        mutual_gradients = torch.autograd.grad(mutual_loss, syn_images)[0]
        self.image_syn.grad += mutual_gradients
        ...
        wandb.log({"Mutual_Loss"+self.tag: mutual_loss.detach().cpu()}, step=it)
    ...
    def evaluate(self, it, testloader, criterion):
        args = self.args
        for model_eval in self.model_eval_pool:
            ...
            for it_eval in range(args.num_eval):
                if args.parall_eva==False:
                    device = torch.device("cuda:0")
                    net_eval = get_network(model_eval, self.channel, self.num_classes, self.im_size, dist=False).to(device) # get a random model
                else:
                    device = args.device
                    net_eval = get_network(model_eval, self.channel, self.num_classes, self.im_size, dist=True).to(device) # get a random model

                eval_labs = self.label_syn.detach().to(device)
                with torch.no_grad():
                    image_save = self.image_syn.to(device)
            ...
            if acc_test_mean > self.best_acc[model_eval]:
                self.best_acc[model_eval] = acc_test_mean
                self.best_std[model_eval] = acc_test_std
                save_this_it = True
            print('Evaluate %d random %s, mean = %.4f std = %.4f\n-------------------------'%(len(accs_test), model_eval, acc_test_mean, acc_test_std))
            # TODO log at different table
            wandb.log({'Accuracy/{}'.format(model_eval)+self.tag: acc_test_mean}, step=it)
            wandb.log({'Max_Accuracy/{}'.format(model_eval)+self.tag: self.best_acc[model_eval]}, step=it)
            wandb.log({'Std/{}'.format(model_eval)+self.tag: acc_test_std}, step=it)
            wandb.log({'Max_Std/{}'.format(model_eval)+self.tag: self.best_std[model_eval]}, step=it)
            
        self.args = args
        return save_this_it

    def save(self, it, isbest=False):
        args = self.args
        with torch.no_grad():
            image_save = self.image_syn.cuda()
            # TODO: save at run_name/ipc
            save_dir = os.path.join(".", "logged_files", args.dataset, str(self.ipc), args.model, wandb.run.name, self.tag)

            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
                
            torch.save(image_save.cpu(), os.path.join(save_dir, "images_{}.pt".format(it)))
            torch.save(self.label_syn.cpu(), os.path.join(save_dir, "labels_{}.pt".format(it)))
            torch.save(self.syn_lr.detach().cpu(), os.path.join(save_dir, "lr_{}.pt".format(it)))

            if isbest:
                torch.save(image_save.cpu(), os.path.join(save_dir, "images_best.pt".format(it)))
                torch.save(self.label_syn.cpu(), os.path.join(save_dir, "labels_best.pt".format(it)))
                torch.save(self.syn_lr.detach().cpu(), os.path.join(save_dir, "lr_best.pt".format(it)))

            wandb.log({"Pixels"+self.tag: wandb.Histogram(torch.nan_to_num(self.image_syn.detach().cpu()))}, step=it)

            if self.ipc < 50 or args.force_save:
                upsampled = image_save
                if args.dataset != "ImageNet":
                    upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=2)
                    upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                wandb.log({"Synthetic_Images"+self.tag: wandb.Image(torch.nan_to_num(grid.detach().cpu()))}, step=it)
                wandb.log({'Synthetic_Pixels'+self.tag: wandb.Histogram(torch.nan_to_num(image_save.detach().cpu()))}, step=it)
                ...
                for clip_val in [2.5]:
                    ...
                    if args.dataset != "ImageNet":
                        ...
                        upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                    grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                    wandb.log({"Clipped_Synthetic_Images/std_{}".format(clip_val)+self.tag: wandb.Image(torch.nan_to_num(grid.detach().cpu()))}, step=it)
                ...
                if args.zca:
                    ...
                    if args.dataset != "ImageNet":
                        ...
                        upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                    grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                    wandb.log({"Reconstructed_Images"+self.tag: wandb.Image(torch.nan_to_num(grid.detach().cpu()))}, step=it)
                    wandb.log({'Reconstructed_Pixels'+self.tag: wandb.Histogram(torch.nan_to_num(image_save.detach().cpu()))}, step=it)
                    for clip_val in [2.5]:
                        std = torch.std(image_save)
                        mean = torch.mean(image_save)
                        upsampled = torch.clip(image_save, min=mean - clip_val * std, max=mean + clip_val * std)
                        if args.dataset != "ImageNet":
                            ...
                            upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                        grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                        wandb.log({"Clipped_Reconstructed_Images/std_{}".format(clip_val)+self.tag: wandb.Image(
                            torch.nan_to_num(grid.detach().cpu()))}, step=it)
        self.args = args

    def get_syn_images(self):
        return copy.deepcopy(self.image_syn.detach()).cuda()
    
    def get_syn_labels(self):
        return copy.deepcopy(self.label_syn.detach()).cuda()
    
    def get_syn_lr(self):
        return copy.deepcopy(self.syn_lr.detach()).cuda()
...
def main(args):
    ...
    distiller1 = DatasetDistiller(args, args.ipc, channel, im_size, num_classes, tag="_1")
    distiller2 = DatasetDistiller(args, args.ipc, channel, im_size, num_classes, tag="_2")

    '''load expert trajectory file '''
    ...
    # del Temp_net
    distiller1.init_synthesis(images_all, indices_class, expert_files, buffer)
    distiller2.init_synthesis(images_all, indices_class, expert_files, buffer)
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            is_best_1 = distiller1.evaluate(it, testloader, criterion)
            is_best_2 = distiller2.evaluate(it, testloader, criterion)
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            distiller1.save(it, is_best_1)
            distiller2.save(it, is_best_2)
            # with torch.no_grad():
        ...
        syn_images_2 = distiller2.get_syn_images()
        syn_labels_2 = distiller2.get_syn_labels()
        syn_lr_2 = distiller2.get_syn_lr()
        distiller1.train_iter(it, expert_trajectory, criterion, syn_images_2, syn_labels_2, syn_lr_2)
        syn_images_1 = distiller1.get_syn_images()
        syn_labels_1 = distiller1.get_syn_labels()
        syn_lr_1 = distiller1.get_syn_lr()
        distiller2.train_iter(it, expert_trajectory, criterion, syn_images_1, syn_labels_1, syn_lr_1)
